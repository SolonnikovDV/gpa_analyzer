...
# ---------------------------------------------------------------------------
# Executor adapter functions — called by modules/analysis/models/{spark,pyspark}
# ---------------------------------------------------------------------------

def analyze_spark(source_text: str, **kwargs: Any) -> Dict[str, Any]:
    analyzer = SparkRuntimeAnalyzer(runtime_config=kwargs.get("runtime_config") or {})
    return analyzer.analyze_with_user_sizes(
        kwargs.get("params", []),
        kwargs.get("user_sizes", {}),
        analysis_mode=kwargs.get("analysis_mode", "logic"),
        plan_source=kwargs.get("plan_source", "none"),
    )


def discover_spark(source_text: str, **kwargs: Any) -> Dict[str, Any]:
    analyzer = SparkRuntimeAnalyzer(runtime_config=kwargs.get("runtime_config") or {})
    return analyzer.discover_tables(source_text)


def analyze_pyspark(source_text: str, **kwargs: Any) -> Dict[str, Any]:
    analyzer = PySparkRuntimeAnalyzer(runtime_config=kwargs.get("runtime_config") or {})
    return analyzer.analyze_with_user_sizes(
        kwargs.get("params", []),
        kwargs.get("user_sizes", {}),
        analysis_mode=kwargs.get("analysis_mode", "logic"),
...
def discover_pyspark(source_text: str, **kwargs: Any) -> Dict[str, Any]:
    analyzer = PySparkRuntimeAnalyzer(runtime_config=kwargs.get("runtime_config") or {})
    return analyzer.discover_tables(source_text)
