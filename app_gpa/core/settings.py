...
def load_project_environment() -> None:
    ...
    for candidate_dir in (PROJECT_ROOT, APP_DIR):
...
class AppSettings:
    ...
    def from_env(cls) -> "AppSettings":
        return cls(
            ...
            runtime_store_dir=os.environ.get("RUNTIME_STORE_DIR", str(PROJECT_ROOT / ".runtime_store")),
            persistence_db_path=os.environ.get(
                "PERSISTENCE_DB_PATH",
                os.path.join(
                    os.environ.get("RUNTIME_STORE_DIR", str(PROJECT_ROOT / ".runtime_store")),
                    "app_state.sqlite3",
                ),
            ),
        )
