...
_APP_GPA = Path(__file__).resolve().parent.parent
_PROJECT_ROOT = _APP_GPA.parent
sys.path.insert(0, str(_APP_GPA))
from core.compat import install_compat_imports

install_compat_imports()

for _d in (_PROJECT_ROOT, _APP_GPA):
...
def _resolve_token(args: argparse.Namespace) -> tuple[str, str]:
    ...
    if not token:
        token = _read_token_from_key(os.path.join(str(_PROJECT_ROOT), ".key"))
    if not token:
...
def _validate_chat(credentials: str, scope: str, model: str) -> tuple[bool, str]:
    from modules.agents.gigachat_agent import _gigachat_client_kwargs
